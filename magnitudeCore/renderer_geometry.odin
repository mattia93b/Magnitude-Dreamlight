...
uploadGeometry::proc(renderer: ^Renderer){
    ...
    for path, i in renderer.textures {
        if i >= 16 do break 
        ...
        renderer.allTextures[i] = sdl.CreateGPUTexture(renderer.gpu.device, sdl.GPUTextureCreateInfo{
            type = .D2,
        ...
        sdl.UploadToGPUTexture(
            ...
            { texture = renderer.allTextures[i], w = cast(u32)surface.w, h = cast(u32)surface.h, d = 1 }, 
            false
        )

        sdl.ReleaseGPUTransferBuffer(renderer.gpu.device, stagingBuffer)
